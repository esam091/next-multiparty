...
function parseForm(req, options) {
  ...
  if (!((_req$headers$content = req.headers['content-type']) != null && _req$headers$content.startsWith('multipart/form-data'))) {
    throw new Error('Invalid Content-Type Header');
  }

  return new Promise(function (resolve, reject) {
    formidable(options).parse(req, function (err, fields, files) {
      if (err) {
        return reject(err);
      }

      var parsedFiles = Object.entries(files).map(function (_ref) {
        var name = _ref[0],
            file = _ref[1];
        var singleFile = Array.isArray(file) ? file[0] : file;
        return _extends({}, singleFile, {
          name: name,
          toBuffer: function toBuffer() {
            return fs.promises.readFile(singleFile.filepath);
          },
          destroy: function destroy() {
            return (// we just ignore the error here because if the file doesn't exist,
              // we don't need to delete it anymore
              fs.promises.unlink(singleFile.filepath)["catch"](function () {})
            );
          }
        });
      });
      var parsedFields = Object.entries(fields).reduce(function (acc, _ref2) {
        var name = _ref2[0],
            value = _ref2[1];
...
function getConfig(config) {
  return _extends({}, config, {
    api: _extends({}, config == null ? void 0 : config.api, {
      bodyParser: false
    })
  });
}
...
function withFileUpload(handler, options) {
  return /*#__PURE__*/function () {
    var _ref = _asyncToGenerator( /*#__PURE__*/runtime_1.mark(function _callee(req, res) {
      var _config$allowedMethod;

      var config, _yield$parseForm, files, fields;

      return runtime_1.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              config = _extends({}, DEFAULT_OPTIONS, options);

              if ((_config$allowedMethod = config.allowedMethods) != null && _config$allowedMethod.includes(req.method)) {
                _context.next = 4;
                break;
              }
            ...
            case 7:
              _context.next = 9;
              return parseForm(req, config.formidableOptions);

            case 9:
              _yield$parseForm = _context.sent;
              files = _yield$parseForm.files;
              fields = _yield$parseForm.fields;
...
export { getConfig, withFileUpload };
//# sourceMappingURL=next-multiparty.esm.js.map
